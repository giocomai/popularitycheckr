...
pc_combine <- function(..., url_column = "landingPagePath", sheet = "combine") {
  ...
  purrr::walk(.x = seq_along(dots),
              .f = function(x) {
                ...
                purrr::walk(.x = unique(current_sheet_df[[url_column]][(is.na(current_sheet_df[[current_field]])|current_sheet_df[[current_field]]=="")&(is.na(current_sheet_df[[url_column]])==FALSE|(is.null(current_sheet_df[[url_column]])==FALSE))]),
                            .f = function(y) {
